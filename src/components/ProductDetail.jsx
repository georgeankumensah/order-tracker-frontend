...
const ProductDetail = () => {
	...
	const [statusName, setStatusName] = useState(["Order Confirmed", "Shipped", "Out For Delivery", "Delivered"]);
	...
	const { name, destination, image, _id, qty, status, currentWarehouse, expectedArrivalTime, createdAt, otherInformation, details } = order;
						...
						<div>
							<ul className="steps font-medium">
								{statusName.map((item, index) => {
									...
									return (
										<li className={`step ${index < statusLength ? "step-info " : ""}`} key={index}>
											{item}
										</li>
									);
								})}
							...
							<p className="font-medium relative mt-3  text-[#0a0a0a] ">
								Package Name : <br /> <span className="font-normal text-xl">{name}</span>{" "}
							</p>
							<br />
							<p className="font-medium text-[#0a0a0a]">
								Quantity : <span className="font-normal text-sm">{qty}</span>
							</p>
							{destination && (
								<>
									<p className="font-medium relative mt-3  text-[#0a0a0a] ">
										From : <span className="font-normal text-sm">{destination.from}</span>
									</p>
									<p className="font-medium relative mt-3  text-[#0a0a0a] ">
										To : <span className="font-normal text-sm">{destination.to}</span>
									</p>
								</>
							)}
							<br />
							...
							<br />
							<p className="font-medium text-[#0a0a0a] ">
								Currently At : <span className="font-normal text-sm">{currentWarehouse || "UNKNOWN"}</span>{" "}
							</p>
							<br />
								...
								{status &&
									status.map((item, index) => {
										...
										return (
											<li key={index}>
												<div className="flex items-center gap-[1rem]">
													<p className="font-normal text-[#0a0a0a]">{item.statusTitle} -</p>
